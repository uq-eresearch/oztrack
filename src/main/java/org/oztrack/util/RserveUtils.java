            ...
            while ( (line = br.readLine()) != null) {
                if (capture) { // we are supposed to capture the output from REG command
                    ...
                    if (i >= 0) {
                        ...
                        installPath = s;
                        logger.debug("R InstallPath = "+s);
                    }
                } else
                    logger.debug("Rserve>" + line);
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
...
public class RserveUtils {
    ...
    public static boolean launchRserve(String cmd, String rargs, String rsrvargs, boolean debug) {
        try {
            ...
            logger.debug("failed to start Rserve process with "+x.getMessage());
            return false;
        }
    ...
    public static boolean checkLocalRserve() {
        ...
        String osname = System.getProperty("os.name");
        if (osname != null && osname.length() >= 7 && osname.substring(0,7).equals("Windows")) {
            logger.debug("Windows: query registry to find where R is installed ...");
            ...
            try {
                ...
                logger.debug("ERROR: unable to run REG to find the location of R: "+rge);
                return false;
            }
            if (installPath == null) {
                logger.debug("ERROR: canot find path to R. Make sure reg is available and R was installed with registry settings.");
                return false;
            }
            return launchRserve(installPath+"\\bin\\R.exe");
        }
        return (launchRserve("R") || /* try some common unix locations of R */
            ((new File("/usr/lib64/R/bin/R")).exists() && launchRserve("/usr/lib64/R/bin/R")) ||
    ...
    public static void main(String[] args) {
        logger.debug("result="+checkLocalRserve());
        try {
            RConnection c=new RConnection();
            c.shutdown();
        } catch (Exception x) {};
